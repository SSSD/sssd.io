...
Please note that for SSSD versions older than 2.9.3 by default memory allocated
with ``NULL`` given as a parent, e.g. ``talloc_new(NULL)`` will not be shown in
the hierarchy. For the SSSD versions newer than SSSD-2.6.0 the hierarchy below
SSSD's internal ``autofree_context`` is printed while for older versions the
hierarchy below the deprecated libtalloc internal ``autofree_context``.
